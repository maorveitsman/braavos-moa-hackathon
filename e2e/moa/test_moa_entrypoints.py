...
@pytest.mark.asyncio
async def test_multicall_non_existing_selector(prepare_simple_signer):
    signer: TestSigner = prepare_simple_signer
    ...
    with pytest.raises((TransactionRevertedError, ClientError)):
        await signer.execute_calls([
            ...
            [int(FEE_CONTRACT_ADDRESS, 16), "non_existent", []],
        ])


@pytest.mark.asyncio
async def test_multicall_malformed_calldata(prepare_simple_signer):
    signer: TestSigner = prepare_simple_signer
    ...
    with pytest.raises((TransactionRevertedError, ClientError)):
        await signer.execute_calls([
            ...
            [int(FEE_CONTRACT_ADDRESS, 16), "balanceOf", []],
        ])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("add_signer", "getPublicKey"),
    ],
    ids=[
        "add_signer, set_multisig",
        "disable_multisig, remove_signer",
        "disable_multisig_with_etd, remove_signer_with_etd",
        "cancel_deferred_remove_signer_req, cancel_deferred_disable_multisig_req",
        "disable_multisig, cancel_deferred_remove_signer_req",
        "cancel_deferred_remove_signer_req, set_multisig",
        "add_signer, getPublicKey",
    ],
...
async def test_multicall_allowed_call_to_self_combinations(
        prepare_simple_signer, selector1, selector2):
    signer: TestSigner = prepare_simple_signer
    ...
    with pytest.raises((TransactionRevertedError, ClientError)):
        await signer.execute_calls([
            ...
            [signer.address, selector1, []],
            [signer.address, selector2, []],
        ])
...
async def test_is_valid_sig_sanity_stark_indexed(prepare_simple_signer):
    ...
    sig_r, sig_s = signer.sign_hash(TEST_HASH)
...
async def test_is_valid_sig_2_sig_correct(prepare_signer):
    ...
    sig_r, sig_s = signer.sign_hash(TEST_HASH)
    sig_r_1, sig_s_1 = signer.sign_hash(TEST_HASH, 1)
    res = await signer.account.functions["is_valid_signature"].call(
        TEST_HASH,
        [
            0,
